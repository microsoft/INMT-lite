...
for key in interface_cluster.keys():  
    if key not in ['B','PE']:
        ...
        for record in interface_cluster[key]:
            ...
            for ts in logs: 
                ...
                if key == 'SBOW':     ## Specific computation for SBOW which does not have the total number of suggestions shown
                    total_suggestions = 0 
                    with open('/home/t-hdiddee/INMT-lite/experimental/data/final_study_dump/user-study-all-task-dump.json') as file:
        ...
        print(f'For interface {key}: the average number of tapped indices by new method {(tapped_suggestions/total_suggestions)*100}')
        print('*******************************************************************************************************************')
    else: 
